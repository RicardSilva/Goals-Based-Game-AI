...
namespace Assets.Scripts.IAJ.Unity.Pathfinding
{
    public class NodeArrayAStarPathFinding : AStarPathfinding
    {
        ...
        protected override void ProcessChildNode(NodeRecord bestNode, NavigationGraphEdge connectionEdge)
        {
            ...
            if ((existingNode = this.Open.SearchInOpen(childNodeRecord)) != null)
            {
                if (existingNode.fValue > f)
                {
                    InitializeChildNode(childNodeRecord, g, h, f, bestNode);
                    this.Open.Replace(existingNode, childNodeRecord);
                }
            }
            // CHECK IF CHILD IS IN CLOSED
            else if ((existingNode = this.Closed.SearchInClosed(childNodeRecord)) != null)
            {
                if (existingNode.fValue > f)
                {
        ...
        private void InitializeChildNode(NodeRecord node, float g, float h, float f, NodeRecord parent)
        {
            node.gValue = g;
            node.hValue = h;
            node.fValue = f;
